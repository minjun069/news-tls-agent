...
수정 호출이 한 번만 발생하는지 검사한다. 첫 라운드 선정 0건은 다음 P3로 진행하며, 라운드별
선정 건수가 같아도 기사 ID가 다르면 `converged`가 아니라 최대 라운드 등 다른 종료 조건을
적용하는지 확인한다.

RH-05 그래프 출력 회귀는 `backend/tests/unit/test_gemini.py`가 구조 검증 오류에 응답 타입,
원본 출력과 잘못된 관계 끝점이 보존되는지 검사한다. `test_graph.py`는 오류 정보를 넣은 교정
호출이 한 번만 발생하는지와 교정도 실패할 때 유효하지 않은 관계만 제외하고 나머지를 기사
단위로 저장하는지 검사한다. 엔티티 구조까지 잘못돼 안전하게 복구할 수 없는 출력은 저장하지
않고 기존 실패 로그를 유지한다.

RH-06 모델 오류 정책 회귀는 `backend/tests/unit/test_gemini.py`에서 404가 1회, 429가
`Retry-After` 대기 뒤 최대 2회, 503이 `1·2·4초 + jitter` 대기와 함께 최대 4회만 호출되는지
