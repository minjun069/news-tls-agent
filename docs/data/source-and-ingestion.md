...
| `converged` | 두 번째 라운드부터 직전 라운드 이후 새 선정 기사 ID가 추가되지 않음 |
...
| `search_no_hits` | 첫 검색과 기간 없는 원 토픽 hybrid 복구 검색 모두 MS-SQL 복원 기사 0건 | 토픽의 기간·핵심어 수정 |
| `selection_rejected_all` | 검색 후보는 있었지만 P4 수정 요청과 다음 검색 뒤에도 누적 선정 기사 0건 | 더 구체적인 토픽 입력 |
| `model_unavailable` | 모델 404 설정 오류 또는 503 재시도 소진 | `retryable`이 true일 때만 재시도 |
| `output_validation_failed` | 구조화 출력을 검증하지 못해 저장하지 않음 | 재시도 가능 |
| `generation_failed` | LLM 호출 실패 또는 저장 실패 | 가능 |
| `rate_limited` | 외부 API 한도 초과 | 가능 |

> 검색·선정 실패와 모델·저장 실패를 구분하는 이유는 사용자의 다음 행동이 다르기 때문이다.
> 전자는 토픽을 고쳐야 하고, 후자는 `retryable`이 true이면 같은 요청을 다시 시도한다.
